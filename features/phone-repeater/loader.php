...
/**
 * Format phone data for display
 * 
 * @param array|string $phone_data Phone data array or JSON string
 * @param string $format Display format: 'full', 'primary_only', 'list'
 * @return string Formatted phone display
 */
function oo_format_phone_display( $phone_data, $format = 'full' ) {
    // Handle JSON string input
    if ( is_string( $phone_data ) ) {
    ...
    if ( ! is_array( $phone_data ) || empty( $phone_data ) ) {
        return '—';
    }
    ...
    foreach ( $phone_data as $phone ) {
        ...
        $phone_display = esc_html( $phone['number'] );
        
        // Add extension if present
        if ( ! empty( $phone['extension'] ) ) {
            $phone_display .= ' ext. ' . esc_html( $phone['extension'] );
        }
        
        // Add type if present and format allows
        if ( ! empty( $phone['type'] ) && $format !== 'primary_only' ) {
            $phone_display = esc_html( $phone['type'] ) . ': ' . $phone_display;
        }
        
        // Add primary indicator
        if ( ! empty( $phone['is_primary'] ) && $format !== 'primary_only' ) {
            $phone_display .= ' <span style="color: #46b450; font-weight: bold;">(Primary)</span>';
        }
        
        // For primary_only format, only show the primary phone
        if ( $format === 'primary_only' ) {
            if ( ! empty( $phone['is_primary'] ) ) {
                return $phone_display;
            }
        } else {
            $output[] = $phone_display;
        }
    }
    
    // If primary_only format and no primary found, show first phone
    if ( $format === 'primary_only' && empty( $output ) && ! empty( $phone_data ) ) {
        $first_phone = reset( $phone_data );
        if ( ! empty( $first_phone['number'] ) ) {
            $phone_display = esc_html( $first_phone['number'] );
            if ( ! empty( $first_phone['extension'] ) ) {
                $phone_display .= ' ext. ' . esc_html( $first_phone['extension'] );
            }
            return $phone_display;
        }
    }
    ...
    return $format === 'list' ? implode( '<br>', $output ) : implode( ', ', $output );
} 