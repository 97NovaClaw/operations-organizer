...
jQuery(document).ready(function($) {
    ...
    $('.oo-tab-content .oo-start-link-btn, .oo-tab-content .oo-stop-link-btn').on('click', function(e) {
        ...
        var url = isAdminUrl + '?page=' + actionPage + '&job_number=' + encodeURIComponent(jobNumber) + '&phase_id=' + encodeURIComponent(phaseId) + '&return_tab=content';
    ...
    $(document).on('click', '.oo-edit-log-action', function(e) {
        ...
        var $modal = $('#edit-log-modal');
        var $kpiFieldsContainer = $modal.find('#edit-log-dynamic-kpi-fields');
        
        // Clear form & dynamic fields
        $('#edit-log-form')[0].reset();
        $kpiFieldsContainer.empty().hide(); 
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    // Populate Dynamic KPI Fields
                    if (logData.phase_kpi_definitions && logData.phase_kpi_definitions.length > 0) {
                        var kpiHtml = '';
                        logData.phase_kpi_definitions.forEach(function(kpi_def) {
                            var currentValue = (logData.recorded_kpis_array && typeof logData.recorded_kpis_array[kpi_def.measure_key] !== 'undefined') ? logData.recorded_kpis_array[kpi_def.measure_key] : '';
                            var inputId = 'edit_kpi_val_' + kpi_def.measure_key;
                            var inputName = 'edit_kpi_values[' + kpi_def.measure_key + ']';
                            var label = esc_html(kpi_def.measure_name) + (kpi_def.is_mandatory == 1 ? ' <span class="required" style="color:red;">*</span>' : '');
                            var inputField = '';

                            switch (kpi_def.unit_type) {
                                case 'integer':
                                case 'decimal':
                                    inputField = '<input type="number" id="' + inputId + '" name="' + inputName + '" value="' + esc_html(currentValue) + '" '
                                                + (kpi_def.unit_type === 'decimal' ? 'step="0.01"' : 'step="1"') 
                                                + ' class="regular-text" />';
                                    break;
                                case 'text':
                                    inputField = '<textarea id="' + inputId + '" name="' + inputName + '" rows="2" class="widefat">' + esc_html(currentValue) + '</textarea>';
                                    break;
                                case 'boolean':
                                    inputField = '<input type="checkbox" id="' + inputId + '" name="' + inputName + '" value="1" ' + (currentValue == '1' || currentValue === true ? 'checked' : '') + ' />';
                                    break;
                                default:
                                    inputField = '<input type="text" id="' + inputId + '" name="' + inputName + '" value="' + esc_html(currentValue) + '" class="regular-text" />';
                            }
                            kpiHtml += '<div class="form-field"><label for="' + inputId + '">' + label + '</label>' + inputField + '</div>';
                        });
                        if (kpiHtml) {
                             $kpiFieldsContainer.html('<h4>' + (oo_data.text_kpi_values || 'KPI Values') + '</h4>' + kpiHtml).show();
                        } else {
                             $kpiFieldsContainer.empty().hide();
                        }
                    } else {
                        $kpiFieldsContainer.html('<p><em><?php echo esc_js(__("No KPIs configured for this phase.", "operations-organizer")); ?></em></p>').show();
                    }
    ...
    function resetButtonStates() {
        ...
        // Also clear dynamic kpi fields from modal - this is handled by clearing $kpiFieldsContainer above
        // $('#edit-log-dynamic-kpi-fields').empty().hide(); 
