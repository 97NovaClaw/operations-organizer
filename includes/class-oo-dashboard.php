...
class OO_Dashboard { // Renamed class
    ...
    public static function ajax_get_dashboard_data() {
        ...
        oo_log('AJAX: Get dashboard data', __METHOD__);
        ...
        $logs_data = OO_DB::get_job_logs(array(
        ...
        if (is_wp_error($logs_data)) {
            oo_log('Error getting logs for dashboard: ' . $logs_data->get_error_message(), __METHOD__);
            wp_send_json_error(['message' => $logs_data->get_error_message()]);
            return;
        }

        $logs = $logs_data['logs'];
        $total_records = $logs_data['total_count'];
        $total_filtered_records = $logs_data['filtered_count'];
        ...
        foreach ($logs as $log) {
            ...
            // Use only JSON data for KPIs (no legacy column handling)
            $current_kpi_values = $kpis_from_json;

            // Generic KPI display
            $kpi_display_array = array();
            // Loop through KPIs and display them
            foreach ($current_kpi_values as $key => $value) {
                if (!empty($value)) {
                    $kpi_display_array[] = ucfirst($key) . ": $value";
                }
            }

            // Get employee name display
            $employee_name = esc_html($log->first_name . ' ' . $log->last_name);
            if (!empty($log->employee_number)) {
                $employee_name .= ' (' . esc_html($log->employee_number) . ')';
            }

            // Get job number (prefer from log, fallback to job table)
            $job_number = !empty($log->job_number) ? $log->job_number : (isset($log->job_number_from_job) ? $log->job_number_from_job : 'N/A');

            // Status badge
            $status_class = ($log->status === 'completed') ? 'oo-status-completed' : (($log->status === 'started') ? 'oo-status-started' : 'oo-status-unknown');
            $status_text = ucfirst($log->status);
            $status_badge = '<span class="' . $status_class . '">' . esc_html($status_text) . '</span>';
    ...
    public static function ajax_update_job_log() {
        ...
        // Handle custom KPI data from JSON field
        if (isset($_POST['edit_log_kpi_data']) && !empty($_POST['edit_log_kpi_data'])) {
            $kpi_json = stripslashes($_POST['edit_log_kpi_data']);
            $decoded_kpi = json_decode($kpi_json, true);
            
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded_kpi)) {
                $data_to_update['kpi_data'] = $kpi_json;
            } else {
                oo_log('AJAX Error: Invalid KPI JSON data: ' . $kpi_json, __METHOD__);
                wp_send_json_error(['message' => 'Invalid KPI JSON data format.']);
                return;
            }
        }
